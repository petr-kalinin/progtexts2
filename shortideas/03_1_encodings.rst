...
Например, в английском у каждой буквы есть две версии и это разные символы: например, I и i.
...
Имя मनीष может визуально выглядеть как три «символа», однако на самом деле न в юникоде записывается
как два символа: сначала न, а потом «дорисовывающий» ी.
Аналогичная конструкция может возникнуть с кириллической буквой «ё»: если не ошибаюсь, на macOS
редакторы регулярно сохраняют её как два символа подряд («е» и «дорисуй две точки») вместо одного.
...
* Code point — то, что выше подразумевалось как «символ»,
  ...
  в зависимости от соседей (как в арабском), некоторые могут с соседями склеиваться,
...
Тем не менее, даже если вы научились выделять из строчки rapheme cluster, это вам не поможет.
Например, если вы «обрежете» текст в середине (например, для предпросмотра), то символы на границе
могут начать выглядеть по-другому и поменяют смысл.
При этом вы не можете обрезать «на границе слов» — в китайском не используются пробелы.
