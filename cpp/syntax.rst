...
не гарантируется.
В тестирующей системе легко может быть выведено ``2``, если компилятор
...
Аналогичная проблема может возникнуть при выводе на экран или вызове функций:
...
Логические операторы вычисляются «лениво», как и в паскале или питоне: в выражении ``i < v.size() && v[i] == 0``
сначала будет проверено условие ``i < v.size()``, и только если оно истинно — программа посмотрит в элемент
массива ``b`` с номером ``i``.
...
логической переменной ``success``, и выполнит ``foo()`` только если ``success == false``.
...
.. note ::
    ...
    ``subtract(next_a(), next_a())`` в примере выше может вернуть как ``+``,
    так и ``-1`` даже в C++17.
