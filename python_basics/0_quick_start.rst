...
.. task::
    Вася купил :math:`N` булочек, а Катя — на :math:`K` булочек больше.
...
Можно было поступить и по-другому: можно было, считав данные, сначала отдельно посчитать, сколько булочек купила Катя::

    kate = n + k

после чего вывести ответ как сумму ``n`` и ``kate``::
...
.. task::
    ...
    Гарантируется, что :math:`N` меньше 1400, т.е. что прошло меньше полных суток.
