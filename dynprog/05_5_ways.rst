...
.. task::
    ...
    (Здесь :math:`m` — количество секунд в рабочем дне, т.е. 32400, а :math:`time[i]`,
    как уже указывалось выше, это 
    время забивания гвоздика, если начать в момент :math:`i`, массив :math:`time`
    легко насчитывается из входных данных.)
